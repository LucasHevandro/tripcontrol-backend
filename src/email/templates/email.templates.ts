...
export function forgotPasswordTemplate(params: {
    token: string;
    appUrl: string;
}): string {
    const content = `
        <h1 style="margin:0 0 8px; font-size:20px; color:${TEXT};">Recuperação de senha 🔐</h1>
        <p style="margin:0 0 16px; font-size:15px; line-height:1.5; color:${MUTED};">
            Você solicitou a redefinição da sua senha no TripControl.
        </p>
        <p style="margin:0; font-size:15px; line-height:1.5; color:${MUTED};">
            Clique no botão abaixo para criar uma nova senha. O link é válido por 15 minutos.
        </p>
        ${button('Redefinir senha', `${params.appUrl}/reset-password?token=${params.token}`)}
        <p style="margin:0; font-size:13px; color:${MUTED};">

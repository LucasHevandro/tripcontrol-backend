...
export async function buildTripReportPdf(
  data: TripReportData,
): Promise<Buffer> {
...
function renderHeader(
  ...
  doc
    ...
    .text('TripControl', MARGINS.left, 25);
  doc
    .font('Helvetica')
    .fontSize(8)
  ...
  doc
    .moveTo(MARGINS.left, 45)
    .lineTo(doc.page.width - MARGINS.right, 45)
...
function renderCover(
  ...
  doc.text(
    `${t.budget}: ${data.trip.budget !== null ? currency.format(data.trip.budget) : t.noBudget}`,
  );
  ...
  if (data.trip.description) {
    ...
    doc
      .fillColor(COLORS.text)
      .fontSize(10)
      .text(data.trip.description, { align: 'justify' });
  }
...
function renderFinancialSummary(
  ...
  statBox(
    ...
    boxWidth,
    t.perPersonAverage,
    currency.format(data.perPersonAverage),
  );
  ...
  if (data.trip.budget !== null) {
    ...
    statBox(
      ...
      boxWidth,
      t.budget,
      currency.format(data.trip.budget),
    );
    statBox(
      doc,
      MARGINS.left + boxWidth + 20,
...
function renderBalances(
  ...
  for (const p of data.participants) {
    ...
    const balanceLabel =
      p.balance > 0.01
        ? `+${currency.format(p.balance)} ${t.positive}`
        : p.balance < -0.01
          ? `${currency.format(p.balance)} ${t.negative}`
